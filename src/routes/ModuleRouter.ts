...
moduleRouter.post("/", controller.createModule.bind(controller));

// PUT - Atualização de módulo (ID na URL, dados no body)
moduleRouter.put("/:id", controller.updateModule.bind(controller));

// DELETE - Exclusão de módulo por ID (via URL param)
moduleRouter.delete("/:id", controller.deleteModule.bind(controller));
